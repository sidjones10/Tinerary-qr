...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: any }> = {
  ...
  dismissed: { label: "Dismissed", color: "text-gray-600 bg-gray-50 dark:bg-gray-900/20", icon: XCircle },
}
...
export default function AdminReportsPage() {
  ...
  const handleReview = async (status: string, action?: string) => {
      ...
      <div className="flex gap-1 bg-white/60 dark:bg-card/60 rounded-xl p-1 border border-[#2c2420]/5 mb-6 overflow-x-auto">
        {(["pending", "reviewed", "resolved", "dismissed", "all"] as const).map((status) => {
          ...
          return (
            ...
            >
              {config ? <config.icon className="h-3 w-3" /> : <Flag className="h-3 w-3" />}
      ...
      {isLoading ? (
        <div className="flex items-center justify-center min-h-[40vh]">
          <div className="text-center">
            ...
            <p className="text-[#2c2420]/60">Loading reports...</p>
          </div>
        </div>
      ) : reports.length === 0 ? (
        <Card className="bg-white/70 dark:bg-card/70 backdrop-blur border-[#2c2420]/5">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-[#2c2420] mb-2">No reports found</h3>
            <p className="text-sm text-[#2c2420]/50 max-w-sm">
              {statusFilter === "pending"
                ? "No pending reports to review. Great job keeping the platform safe!"
                : `No ${statusFilter === "all" ? "" : statusFilter} reports to display.`}
            </p>
          </CardContent>
        </Card>
      ) : (
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-start gap-4">
                    {/* Itinerary thumbnail */}
                    <div className="shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-gradient-to-br from-[#ffb88c]/20 to-[#ff9a8b]/20">
                      {itinerary?.image_url ? (
                        <img
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <Flag className="h-6 w-6 text-[#d97a4a]/50" />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <div>
                          <h3 className="text-sm font-semibold text-[#2c2420] truncate">
                            {itinerary?.title || "Deleted Itinerary"}
                          </h3>
                          <p className="text-xs text-[#2c2420]/40">
                            by {owner?.name || owner?.username || owner?.email?.split("@")[0] || "Unknown"}
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <Avatar className="h-5 w-5">
                          ...
                          <AvatarFallback className="bg-[#ffb88c]/20 text-[#d97a4a] text-[8px]">
                            {(reporter?.name || reporter?.email || "?")[0]?.toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                        <span className="text-xs text-[#2c2420]/50">
                          Reported by {reporter?.name || reporter?.username || reporter?.email?.split("@")[0] || "Unknown"}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      <Dialog open={reviewModalOpen} onOpenChange={setReviewModalOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Take action on this reported itinerary
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedReport && (
            <div className="space-y-4 mt-2">
              ...
              <div className="bg-gray-50 dark:bg-gray-900/30 rounded-lg p-4 space-y-3">
                <div>
                  <p className="text-xs font-medium text-[#2c2420]/40 uppercase">Itinerary</p>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-sm font-semibold text-[#2c2420]">
                      {selectedReport.itinerary?.title || "Deleted"}
                    </p>
                    {selectedReport.itinerary && (
                      <Link
                        href={`/event/${selectedReport.itinerary.id}`}
                        target="_blank"
                        className="text-[#ffb88c] hover:text-[#d97a4a]"
                  ...
                  <p className="text-xs text-[#2c2420]/50 mt-0.5">
                    by {selectedReport.itinerary?.owner?.name || "Unknown"}
                  </p>
                </div>

                <div>
                  <p className="text-xs font-medium text-[#2c2420]/40 uppercase">Reason</p>
                ...
                {selectedReport.description && (
                  <div>
                    <p className="text-xs font-medium text-[#2c2420]/40 uppercase">Details</p>
                    <p className="text-sm text-[#2c2420]/70 mt-0.5">{selectedReport.description}</p>
                  </div>
                )}

                <div>
                  <p className="text-xs font-medium text-[#2c2420]/40 uppercase">Reported by</p>
                  <p className="text-sm text-[#2c2420]/70 mt-0.5">
                    {selectedReport.reporter?.name || selectedReport.reporter?.email || "Unknown"} · {formatDate(selectedReport.created_at)}
                  </p>
                </div>

                {selectedReport.reviewer && (
                  <div>
                    <p className="text-xs font-medium text-[#2c2420]/40 uppercase">Previously reviewed by</p>
                    <p className="text-sm text-[#2c2420]/70 mt-0.5">
                      {selectedReport.reviewer.name || selectedReport.reviewer.username} · {selectedReport.reviewed_at ? formatDate(selectedReport.reviewed_at) : ""}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium text-[#2c2420]/40 uppercase">Actions</p>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleReview("resolved", "make_private")}
                    disabled={isUpdating}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (confirm("Are you sure you want to delete this itinerary? This cannot be undone.")) {
                        handleReview("resolved", "delete_itinerary")
                      }
                    }}
                    disabled={isUpdating}
                  >
              ...
              {isUpdating && (
                <div className="flex items-center justify-center py-2">
                  ...
                  <span className="text-sm text-[#2c2420]/60">Updating...</span>
                </div>
              )}
            </div>
          )}
