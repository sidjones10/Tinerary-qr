...
/**
 * Send event planning email encouraging users to create and share events
 */
export async function sendEventPlanningEmail(params: {
  email: string
  name?: string
}) {
  try {
    ...
    const subject = "Do you have a new event coming up?"
    const resend = getResendClient()
    const { data: resendData } = await resend.emails.send({
      from: FROM_EMAIL,
      to: email,
    ...
    await logEmail({ recipientEmail: email, emailType: "event_planning", subject, status: "sent", resendId: resendData?.id })
    return { success: true }
  } catch (error: any) {
    console.error("Error sending event planning email:", error)
    await logEmail({ recipientEmail: email, emailType: "event_planning", subject: "Do you have a new event coming up?", status: "failed", errorMessage: error.message })
    return { success: false, error: error.message }
  }
}
